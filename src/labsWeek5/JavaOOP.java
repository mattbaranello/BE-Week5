...
public class JavaOOP {
	...
	public static void main(String[] args) {
		...
		CardsClass card = new CardsClass("Three", "Clubs", 6);
		...
		// Test your method here
		CardsClass drawnCard = deck.draw();
		drawnCard.describe();
		...
		Map<String, List<CardsClass>> gameBoard = new HashMap<>();
		int numOfPlayers = 4;
		gameBoard = createGame(numOfPlayers);
		...
		for (int i = 1; i <= numOfPlayers; i++) {
			...
			List<CardsClass> playerList = gameBoard.get(playerName);
			for (CardsClass handCard : playerList) {
				handCard.describe();
			}
			System.out.println("--------------------------");
		}
	...
	// Method 5:
	public static Map<String, List<CardsClass>> createGame(int numOfPlayers) {
		Map<String, List<CardsClass>> finalGameBoard = new HashMap<>();
		Deck deck = new Deck();
		deck.shuffle();
		System.out.println("Deck has " + deck.getCards().size() + " cards.");
		
		for (int i = 1; i <= numOfPlayers; i++) {
			List<CardsClass> playerList = new ArrayList<>();
			String playerName = "Player " + i;
		...
		for (int i = 0; i < 52/numOfPlayers; i++) {
			for (int j = 1; j <= numOfPlayers; j++) {
				...
				List<CardsClass> playerList = finalGameBoard.get(playerName);
